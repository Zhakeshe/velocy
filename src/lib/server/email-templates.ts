...
function baseTemplate({ title, body, footer }: TemplateBase) {
  const defaultFooter = `
    <div>
      <p class="text">Телеграм канал: <a href="https://t.me/velocyllc" style="color:#47d7a3; text-decoration:none;">t.me/velocyllc</a></p>
      <div class="divider"></div>
      <p class="text"><strong>Условия использования</strong></p>
      <p class="text">
        Используя сервис Velocy, вы подтверждаете, что предоставляете достоверные данные, соблюдаете законы вашей страны
        и не используете сервис для вредоносной активности, спама, атак или распространения запрещенного контента.
        Оплаты считаются окончательными после зачисления, а доступ к услугам может быть ограничен при нарушении правил,
        угрозе безопасности или по запросу компетентных органов.
      </p>
      <p class="text">
        Вы обязуетесь хранить доступы к аккаунту в безопасности. Ответственность за действия в аккаунте несет владелец.
        Мы можем обновлять условия в любое время, публикуя обновления в этом письме и личном кабинете.
      </p>
      <p class="text"><strong>Политика конфиденциальности</strong></p>
      <p class="text">
        Мы собираем только данные, необходимые для работы сервиса: email, имя, платежные данные и параметры заказанных услуг.
        Данные используются для предоставления услуг, уведомлений и защиты аккаунта. Мы не продаем ваши данные третьим лицам.
      </p>
      <p class="text">
        Мы можем передавать данные только партнерам, которые обеспечивают оплату, доставку писем и инфраструктуру сервиса,
        и только в объеме, необходимом для выполнения обязательств. Вы можете запросить удаление данных через поддержку.
      </p>
    </div>
  `;
  ...
  <html lang="ru">
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">${footer ?? defaultFooter}</div>
      </div>
    </body>
  </html>
  `;
